...
import com.krish.assignment.adapters.TabOneAdapter;
import com.krish.assignment.R;
import com.krish.assignment.api_responses.example_one.Article;
...
public class FragmentOne extends Fragment implements SwipeRefreshLayout.OnRefreshListener, OnResponseListener<ExampleOneResponse> {

    List<Article> mList=new ArrayList<>();
    RecyclerView mRecyclerView;
    TabOneAdapter listAdapter;
    ...
    public void callDemoAPI() {
        String[] parameters={"apple","2019-03-26","2019-03-26","popularity","efff147ad31d4ce3b42ec99d9de81ea4"};
        if (Utilities.isConnectedToInternet(mContext)) {
            toggleVisibility(true, mProgressBar);
            WebServices<ExampleOneResponse> webServices = new WebServices<ExampleOneResponse>(this);
            webServices.callDemoAPIOne(Utilities.getBaseURL(), WebServices.ApiType.demoapi1,parameters);
    ...
    @Override
    public void onResponse(ExampleOneResponse response, WebServices.ApiType URL, boolean isSucces, int code) {
        switch (URL)
        {
            case demoapi1:
                ...
                if(isSucces)
                {
                    ExampleOneResponse exampleResponse= (ExampleOneResponse) response;
                    if(exampleResponse!=null)
                    {
                        if(exampleResponse.getArticles()!=null)
                        {
                            ...
                            listAdapter=new TabOneAdapter(mContext,mList,mRecyclerView);
